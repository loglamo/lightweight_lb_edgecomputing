...
file_dir = './datasets/intel/interpolated/injected_mixed_experiment/light1.txt'
id_data = [1,2] #1: temp, 2: light 
id_node = [1, 2, 3] #1: sensor node 1, 2: sensor node 2, 3: sensor node 3
id_data_now = id_data[1]  #data set is light data
id_node_now = id_node[0]  #node 1
...
topic = client.topics['sensordata1']              #connect to the topic of sensordata1
...
for item in range(len(sensorData)):
    data_item = {}
    ...
    time.sleep(8)                                #send each 3 seconds
